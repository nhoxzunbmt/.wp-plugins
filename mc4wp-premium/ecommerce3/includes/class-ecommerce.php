...
class MC4WP_Ecommerce {
    ...
    public function update_product( $product ) {
        ...
        if( $product instanceof WC_Product_Variation ) {
            throw new Exception( sprintf( "#%d is a variation of another product. Use the variable parent product instead.", $product->get_id() ) );
        }

       $data = $this->transformer->product( $product );

        return $this->is_object_tracked( $product->get_id() ) ? $this->product_update( $product, $data ) : $this->product_add( $product, $data );
    }
    ...
    private function product_add( WC_Product $product, array $data ) {
        ...
        update_post_meta( $product->get_id(), self::META_KEY, date( 'c' ) );
    ...
    private function product_update( WC_Product $product, array $data ) {
        ...
        try {
            ...
            if( method_exists( $api, 'update_ecommerce_store_product' ) ) {
                $response = $api->update_ecommerce_store_product( $store_id, $product->get_id(), $data );
            } else {
                // FALLBACK: update each variant individually
                foreach ($data['variants'] as $variant_data) {
                    $response = $api->add_ecommerce_store_product_variant( $store_id, $product->get_id(), $variant_data );
                 }
            }
        } catch( MC4WP_API_Resource_Not_Found_Exception $e ) {
