...
class Admin {
		...
		if( $data === null ) {
			...
			try {
				...
				if( ! empty( $license->token ) ) {
					...
					$data->sections['changelog'] = '<p>' . 'You will need to activate your license to install this plugin update.' . '</p>' . $data->sections['changelog'];
					$data->upgrade_notice = 'You will need to activate your license to install this plugin update.';
					$data->package = '';
				}
			} catch( Exception $e ) {
				$data = (object) array();
	...
	public function after_plugin_row() {
		...
		echo sprintf( 'Please <a href="%s">activate your MailChimp for WordPress Premium license</a> to get plugin updates.', admin_url( 'admin.php?page=mailchimp-for-wp-other' ) );
		echo '</td>';
		echo '</tr>';
	}
