								...
								<div class='evcal_db_data'>
									...
									<?php
										...
										// Get all available organizer terms
											$terms = get_terms('event_organizer', array(
												'orderby'           => 'name', 
											    'order'             => 'ASC',
											    'hide_empty'=>false)
											);
									...
									<div class='evoselectfield_saved_data' style='display:<?php echo $evo_organizer_tax_id?'none':'';?>'>
										<?php
											foreach(apply_filters('evo_organizer_pm_fields', array(
												'evcal_organizer_name'=>array(
													'evcal_organizer', 
													__('Event Organizer Name','eventon'), 
													__('eg. Blue Light Band','eventon'),
													(!empty($organizer_terms[0])? $organizer_terms[0]->name:''),
													'text'
												),'evcal_org_contact'=>array(
													'evcal_org_contact', 
													__('(Optional) Organizer Contact Information','eventon'), 
													__('eg. noone[at] thismail.com','eventon'),
													$this->termmeta($termMeta,'evcal_org_contact'),
													'text'
												),'evcal_org_address'=>array(
													'evcal_org_address', 
													__('(Optional) Organizer Address','eventon'), 
													__('eg. 123 Everywhere St., Neverland AB','eventon'),
													$this->termmeta($termMeta,'evcal_org_address'),
													'text'
												),'evcal_org_exlink'=>array(
													'evcal_org_exlink', '','','','link'
												),'evo_metafield_image'=>array(
													'evo_metafield_image', '','','','image'
												)
											)) as $key=>$val){
												if($val[4]=='text'){
													?>
													<p><input type='text' id='<?php echo $key;?>' name='<?php echo $val[0];?>' value="<?php echo $val[3]; ?>" style='width:100%' placeholder='<?php echo $val[2];?>'/><label for='<?php echo $val[0];?>'><?php echo $val[1]; ?></label></p>	
													<?php
												}
												if($val[4]=='link'){
													?>
								...
								<div class='evcal_db_data'>
									...
									<?php
										...
										// GET all available location terms
											$terms = get_terms('event_location', array(
												'orderby'           => 'name', 
											    'order'             => 'ASC',
											    'hide_empty'=>false) 
											);
									...
									<?php
										...
										foreach(array(
											'evo_access_control_location'=>array('evo_access_control_location',__('Make location information only visble to logged-in users','eventon')),
								...
								<div id='evcal_rep' class='evd'>
									...
									<div id='evo_editevent_repeatevents' class='evcalr_2 evo_repeat_options' style='display:<?php echo $display ?>'>
										...
										<div class='repeat_series'>
											...
											<p id='_evcal_rep_series_clickable' class='yesno_row evo '>
												<?php 	
												$_evcal_rep_series_clickable = evo_meta($ev_vals, '_evcal_rep_series_clickable');
												$display = evo_meta_yesno($ev_vals, '_evcal_rep_series_clickable','yes','','none');
		...
		function eventon_save_meta_data($post_id, $post){
			...
			// $_POST FIELDS array
				$fields_ar =apply_filters('eventon_event_metafields', array(
					...
					'evcal_repeat','_evcal_rep_series','_evcal_rep_series_clickable','evcal_rep_freq','evcal_rep_gap','evcal_rep_num',
