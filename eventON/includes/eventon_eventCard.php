				...
				case 'repeats':
					$OT.="<div class='evo_metarow_repeats evorow evcal_evdata_row bordb evcal_evrow_sm ".$end_row_class."'>
						...
						foreach($object->future_intervals as $key=>$interval){
							$OT.="<span data-repeat='{$key}' class='evo_repeat_series_date'>". date($object->date_format.' '.$object->time_format, $interval[0])."</span>";
						}
					$OT.="</p></div>".$end."</div>";
				break;
			...
			// learnmore ICS and close button
				case 'learnmoreICS':				
					...
					// Initial 
						...
						$unix_start = $object->estart;
						$unix_end = $object->eend;
						...
						$__googlecal_link = eventon_get_addgoogle_cal($object, $unix_start, $unix_end);
