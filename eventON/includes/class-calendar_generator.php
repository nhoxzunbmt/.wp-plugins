	...
	/** GENERATE individual event data	for event list array */
		public function generate_event_data(
			...
			// Initial variables
				...
				// schema data
					$show_schema = (!empty($this->evopt1['evo_schema']) && $this->evopt1['evo_schema']=='yes')? false: true;
				...
				// EACH DATA FIELD
							...
							if($repeat_intervals){
								...
								if(count($future_intervals)>0){
									...
									$_eventcard['repeats'] = array(
										...
										'clickable'=>(!empty($ev_vals['_evcal_rep_series_clickable']) && $ev_vals['_evcal_rep_series_clickable'][0]=='yes'?true:false)
									);
								}
							}
						}
					...
					// GOOGLE maps
						if( ($this->google_maps_load) && ($location_address || !empty($lonlat)) &&  
							...
							// GET directions
							if($location_address)	$_eventcard['getdirection'] = array('fromaddress'=>$location_address);
